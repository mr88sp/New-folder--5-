...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    $id = $_POST["id"];
    $current_image = $_POST["current_image"]; // Get current image path from hidden field
    ...
    // Handle image upload
    $new_image_path = $current_image; // Default to current image
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
        ...
        if (in_array($file_extension, $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                ...
                if(!empty($current_image) && file_exists($current_image) && $current_image != $new_image_path){
    ...
    if (empty($name_err) && empty($price_err) && empty($image_err)) {
        ...
        $sql = "UPDATE products SET name=?, description=?, price=?, image_path=?, brand=?, thickness=?, dimensions=?, stock_status=?, badge=?, type=?, category_id=? WHERE id=?";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssisssssssii", $param_name, $param_description, $param_price, $param_image_path, $param_brand, $param_thickness, $param_dimensions, $param_stock_status, $param_badge, $param_type, $param_category_id, $param_id);
...
<div class="max-w-3xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-3xl font-bold text-gray-800">ویرایش محصول</h2>
    ...
    <div class="bg-white rounded-button shadow-sm border border-gray-100 overflow-hidden p-8">
        <p class="text-gray-500 mb-6">لطفا فرم زیر را برای ویرایش محصول اصلاح کنید.</p>

        <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" class="w-full px-4 py-3 rounded-button border border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition duration-200 <?php echo (!empty($name_err)) ? 'border-red-500 ring-1 ring-red-500' : ''; ?>" value="<?php echo $name; ?>">
            ...
            <div>
                ...
                <textarea name="description" rows="4" class="w-full px-4 py-3 rounded-button border border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition duration-200 <?php echo (!empty($description_err)) ? 'border-red-500 ring-1 ring-red-500' : ''; ?>"><?php echo $description; ?></textarea>
            ...
            <div>
                ...
                <input type="text" name="price" class="w-full px-4 py-3 rounded-button border border-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition duration-200 <?php echo (!empty($price_err)) ? 'border-red-500 ring-1 ring-red-500' : ''; ?>" value="<?php echo $price; ?>">
                <span class="text-red-500 text-xs mt-1 block"><?php echo $price_err; ?></span>
            </div>

            <!-- Image -->
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2" for="image">تصویر محصول (در صورت عدم انتخاب، تصویر فعلی باقی می‌ماند)</label>
                ...
                <?php if(!empty($current_image)): ?>
                    <div id="current-image-preview" class="mt-4 text-center">
                        <p class="text-sm text-gray-500 mb-2">تصویر فعلی:</p>
                        <img src="<?php echo htmlspecialchars($current_image); ?>" alt="Current Image" class="max-h-48 mx-auto rounded-button shadow-sm border border-gray-200">
                    </div>
                <?php endif; ?>
                <div id="image-preview" class="mt-4 hidden text-center">
                    <p class="text-sm text-gray-500 mb-2">پیش‌نمایش تصویر جدید:</p>
            ...
            <!-- Submit Button -->
            <div class="pt-4">
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-button shadow transition duration-300 flex justify-center items-center transform hover:-translate-y-0.5">
